...
export const calculateTimeMidpoint = async (
  ...
  // Use optimized versions that implement caching and batching
  const {
  ...
  } = await import('./optimizedMidpointScenarios');
  ...
  try {
    ...
    try {
      ...
      if (error === "REQUEST_DENIED" || 
          ...
          error.message === "API key not authorized for Directions API") {
        return {
          ...defaultResult,
          apiKeyError: true,
...
// Search for places near the midpoint - using cached service
export const findNearbyPlaces = async (
  ...
  try {
    // Use cached places service
    const results = await cachedPlacesService.nearbySearch(
      {
        location: midpoint,
        radius: radius,
        type: placeType,
...
// Get route to a specific place - using cached service
export const getRouteToPlace = async (
  ...
  try {
    // Use cached directions service
    return await cachedDirectionsService.route(
      {
        origin: origin,
